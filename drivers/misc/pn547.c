...
static irqreturn_t pn547_dev_irq_handler(int irq, void *dev_id)
{
		...
		if (!gpio_get_value(pn547_dev->irq_gpio)) {
				#if NFC_DEBUG
				pr_err("%s, irq_gpio = %d\n", __func__, gpio_get_value(pn547_dev->irq_gpio));
				#endif
				return IRQ_HANDLED;
		}
...
static int pn547_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
		...
		if(client->dev.of_node){
				irq_gpio = of_get_named_gpio_flags(client->dev.of_node, "irq-gpio", 0, NULL); 
				ven_gpio = of_get_named_gpio_flags(client->dev.of_node, "ven-gpio", 0, NULL); 
				dl_gpio = of_get_named_gpio_flags(client->dev.of_node, "dl-gpio", 0, NULL); 
    }
    else{
    		dev_err(&client->dev, "failed to load devices\n");
        return -ENODEV;
    }
    
    printk("    irq_gpio[%d], ven_gpio[%d], firm_gpio[%d]\n", irq_gpio, ven_gpio, dl_gpio);
        
		ret = gpio_request(irq_gpio, "nfc_int");
		if (ret)
				return -ENODEV;
		ret = gpio_request(ven_gpio, "nfc_ven");
		if (ret)
				goto err_ven;
		ret = gpio_request(dl_gpio, "nfc_firm");
		if (ret)
				goto err_firm;
		
		gpio_tlmm_config(GPIO_CFG(irq_gpio, 0, GPIO_CFG_INPUT, GPIO_CFG_PULL_DOWN, GPIO_CFG_2MA), GPIO_CFG_ENABLE);
    gpio_tlmm_config(GPIO_CFG(dl_gpio, 0, GPIO_CFG_OUTPUT, GPIO_CFG_NO_PULL, GPIO_CFG_2MA), GPIO_CFG_ENABLE);
    gpio_tlmm_config(GPIO_CFG(ven_gpio, 0, GPIO_CFG_OUTPUT, GPIO_CFG_NO_PULL, GPIO_CFG_2MA), GPIO_CFG_ENABLE);
		...
		*/
		...
		ret = request_irq(client->irq, pn547_dev_irq_handler, IRQF_TRIGGER_RISING, "pn547", pn547_dev);
		if (ret) {
