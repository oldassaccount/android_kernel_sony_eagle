...
//S [CCI]Ginger modified for MSM8226 DTS
/*
void sensor_power_on(struct i2c_client *client)
{
...
/*Kionix Auto-Cali Start*/
		...
		//k printk("KXTJ2 Raw Dataafter,%d,%d,%d==> Z+=%d  Z-=%d \n",raw[0],raw[1],raw[2],Z_AVG[0],Z_AVG[1]);
				...
				struct device_attribute *attr, char *buf)
{
	...
	pid = i2c_smbus_read_byte_data(acceld->client, ACCEL_WHO_AM_I);
...
{
#if 0  //[CCI]Ginger modified for MSM8226 DTS
	const struct kionix_accel_platform_data *accel_pdata = client->dev.platform_data;
#endif
	const struct kionix_accel_platform_data *accel_pdata = &kionix_accel_pdata;
...
//S [CCI]Ginger modified for MSM8226 DTS
static struct of_device_id kionix_accel_dts_table[] = {
...
static struct i2c_driver kionix_accel_driver = {
	.driver = {
		...
		.owner	= THIS_MODULE,
		.of_match_table = kionix_accel_dts_table,  //[CCI]Ginger modified for MSM8226 DTS
	},
...
static int __init kionix_accel_init(void)
{
    ...
    if(g_compass_product_id==6)  //[CCI]Ginger modified 2nd source,MEMSIC Compass Only
