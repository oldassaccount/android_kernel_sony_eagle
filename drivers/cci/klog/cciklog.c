...
#define MSM_KLOG_MAGIC		(MSM_KLOG_HEADER + KLOG_HEADER_LENGTH)
...
//hw information
	...
	0,//model_id
	0,//nfc_id
	"msm8x26",//main_chip[MAIN_CHIP_INFO_LENGTH]
			...
			if(fault_level == 0x5 && strcmp(fault_msg, "modem") == 0)//modem subsystem
			{
				if(modem_log_addr_inited == 1)
				{
					if(modem_log_addr >= MSM_SHARED_RAM_PHYS && modem_log_addr < (MSM_SHARED_RAM_PHYS + MSM_SHARED_RAM_SIZE))//modem dynamic allocated physical address
					{
						...
						modem_log = (void *)MSM_SHARED_RAM_BASE + (modem_log_addr - MSM_SHARED_RAM_PHYS);
					}
					else if(modem_log_addr < MSM_SHARED_RAM_SIZE)//offset
					{
...
//hw_id
	...
	if(magic_update == 1 && device_info_update == 0)
	{
		...
		snprintf(klog_magic + KLOG_INFO_LENGTH + KLOG_IMAGE_INFO_LENGTH, KLOG_HW_ID_LENGTH, "%01X%01X%01X%01X%01X", get_cci_hw_id(), get_cci_project_id(), get_cci_customer_id(), get_cci_band_id(), get_cci_sim_id());
		kprintk("hw_info:hw_id=%01X, project_id=%01X, customer_id=%01X, band_id=%01X, sim_id=%01X\n", get_cci_hw_id(), get_cci_project_id(), get_cci_customer_id(), get_cci_band_id(), get_cci_sim_id());
	}
#endif // #ifdef CONFIG_CCI_HARDWARE_ID
#endif // #ifdef CCI_HW_ID
...
#ifdef CCI_HW_ID
#ifdef CONFIG_CCI_HARDWARE_ID
						snprintf(sysinfo.hw_id, KLOG_HW_ID_LENGTH, "%01X%01X%01X%01X%01X", get_cci_hw_id(), get_cci_project_id(), get_cci_customer_id(), get_cci_band_id(), get_cci_sim_id());
#endif // #ifdef CONFIG_CCI_HARDWARE_ID
#endif // #ifdef CCI_HW_ID
...
#endif // #ifdef CCI_KLOG_DETAIL_LOG
				memcpy(klog_header + KLOG_HEADER_LENGTH + KLOG_MAGIC_TOTAL_LENGTH + KLOG_KERNEL_TIME_LENGTH + KLOG_FIRST_RTC_TIMESTAMP_LENGTH + KLOG_LAST_RTC_TIMESTAMP_LENGTH, &sysinfo.normal_boot, sizeof(struct system_information) - (KLOG_HEADER_LENGTH + KLOG_MAGIC_TOTAL_LENGTH + KLOG_KERNEL_TIME_LENGTH + KLOG_FIRST_RTC_TIMESTAMP_LENGTH + KLOG_LAST_RTC_TIMESTAMP_LENGTH));
			}
			break;
...
#ifdef CCI_HW_ID
#ifdef CONFIG_CCI_HARDWARE_ID
			deviceinfo.hw_id = get_cci_hw_id();
			deviceinfo.band_id = get_cci_band_id();
			deviceinfo.model_id = get_cci_project_id() * 100 + get_cci_customer_id() * 10 + get_cci_sim_id();
			deviceinfo.nfc_id = 1;
#endif // #ifdef CONFIG_CCI_HARDWARE_ID
#endif // #ifdef CCI_HW_ID
...
Ver		1.12.0		Jimmy implement KLog common header file and support subsystem silent restart
Ver		1.13.0		Jimmy implement KLog header (signature + versions)
Ver		1.14.0		Jimmy implement KLog support restoration
*/
