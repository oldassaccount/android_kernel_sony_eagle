...
struct yas_module {
	...
	int startup_time;
	uint8_t odr;
//	struct yas_vector offset;//[CCI]Ginger add for factory test
...
static uint8_t accel_product_id=0;//[CCI]Ginger modified for factory test
static int
yas_init(void)
{
	uint8_t id = 0;  //[CCI]Ginger modified for factory test
...
//		raw->xyz.v[i] -= module.offset.v[i]; //[CCI]Ginger add for factory test
	}
...
static IIO_DEVICE_ATTR(ping, S_IRUGO|S_IWUSR|S_IWGRP,
		yas_ping, NULL, 0);
//static IIO_DEVICE_ATTR(cali, S_IRUGO|S_IWUSR|S_IWGRP,
//		yas_cali, NULL, 0);
...
extern uint8_t g_iio_compass_product_id;//[CCI]Ginger modified 2nd source
static int __init yas_initialize(void)
{
	if(g_iio_compass_product_id==2) //[CCI]Ginger modified 2nd source
